...
Encoding.prototype.encodeAddressIndexKey = function(address, height, txid, index, input, timestamp, send, received) {
  ...
  var sendBuffer = new Buffer(1);
  sendBuffer.writeUInt8(send);
  buffers.push(sendBuffer);

  var receivedBuffer = new Buffer(1);
  receivedBuffer.writeUInt8(received);
...
Encoding.prototype.decodeAddressIndexKey = function(buffer) {
  ...
  var send = buffer[buffer.byteLength -2]
  var received = buffer[buffer.byteLength -1]
  return {
    address: address,
    height: height,
    txid: txid,
