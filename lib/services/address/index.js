  ...
  function getTxList(next) {
    ...
    async.eachLimit(addresses, 4, function(address, next) {
      ...
      var list = lodash.uniqBy(options.txIdList, function(x) { 
        return x.txid + x.height + x.send + x.received;
      });

      if(options.filter === 'send'){
        list = lodash.filter(list,function (item){
          return item.send === 1;
        })
        ...
        list = lodash.filter(list,function (item){
          return item.height !== 0xffffffff;
        })
      } else if (options.filter === 'unconfirmed') {
        list = lodash.filter(list, function (item) {
          return item.height === 0xffffffff;
        })
      }
...
AddressService.prototype.getAPIMethods = function() {
  return [
    ['getAddressHistory', this, this.getAddressHistory, 3],
...
AddressService.prototype._processInput = function(tx, input, index, opts) {
  ...
  // address index
  var addressKey = this._encoding.encodeAddressIndexKey(address, opts.block.__height, txid, index, 1, timestamp, 1, 0);
...
AddressService.prototype._processOutput = function(tx, output, index, opts) {
  ...
  var addressKey = this._encoding.encodeAddressIndexKey(address, opts.block.__height, txid, index, 0, timestamp, 0, 1);
